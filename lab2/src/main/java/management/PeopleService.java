...
public class PeopleService {
  ...
  public PeopleService(String pathToDirectory, String pathToIdsFile) {
    psDao =
        new PeopleDAO(pathToIdsFile, pathToDirectory); // optional, use PeopleDAO or CachedPeopleDAO
  }
  ...
  public void processTheCommand(Command cmd) throws Exception {
    ...
    switch (cmd.getCommandSpecifier()) {
      ...
      case "CS":
        ...
        Student student =
            new Student(
                cSC.getFullName(), cSC.getYearOfBirth(), cSC.getTelephoneNumber(), cSC.getGrades());
      ...
      case "MS":
        ...
        if (tempPerson.getClass() != Student.class) {
          throw new Exception(
              "Attempt to modify the teacher using the no-teacher modify command\n"
                  + "\nTeacher:\n"
      ...
      case "MT":
        ...
        if (tempPerson1.getClass() != Teacher.class) {
          throw new Exception(
              "Attempt to modify the student using the no-student modify command\n"
                  + "\nStudent:\n"
      ...
      default:
        throw new Exception(
            "Untracked Specifier: " + cmd.getCommandSpecifier() + "\nCommand:\n" + cmd);
    }
  }
  ...
  public void ShowPeopleId() {
    if (psDao.getClass() == CachedPeopleDAO.class) {
      for (Long id : ((CachedPeopleDAO) psDao).getPeopleById().keySet()) {
        System.out.println(
            "ID " + id + ": " + ((CachedPeopleDAO) psDao).getPeopleById().get(id).getFullName());
      }
    } else {
